...
class AddOrEdit extends Component {
    ...
    navigationButtonPressed({ buttonId }) {
        ...
        if (!this.state.unitName) {
            ...
            MyToast.info('施药面积应大于0，且最多两位小数的数字');
            return false
        } else if (!ret.test(this.state.safeDays)) {
            MyToast.info('施药面积应大于0，且最多两位小数的数字');
            return false
        }
        let input = {
                                ...
                                <View>
                                    {
                                        goodsData.map((item, index) => {
                                            return <View style={styles.bgBorder}>
                                                ...
                                                <View style={styles.rows}><Text>农药类型：{item.categoryName ? item.categoryName : '--'}{item.categoryCode ? item.categoryCode : '--'}</Text></View>
                                                <View style={styles.rowsLine}>
