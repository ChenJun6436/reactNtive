...
@navigatorDecorator
class IntBuyRecord extends Component {
    constructor(props) {
        super(props);
        this.state = {
            publishState: [''],
                                ...
                                onPress: () => {
                                    Alert.alert(
                                        ...
                                        },
                                        {
                                            text: '确定',
                                            onPress: () => {
                                                MineAction.DeleteBuy([item.id]).then((data) => {
                                                    if (data.suc) {
                                                        this.refresh()
                                                    } else {
                                ...
                                onPress={() => {
                                    this.pushPage({
                                        component: {
                                            ...
                                            ...Global.Screens.IntBuyRecordDetail,
                                        }
                                    });
                                }}>
                                <View style={{ borderBottomColor: '#e3e3e3', borderBottomWidth: 1, paddingLeft: 12, paddingRight: 12, paddingTop: 10 }}>
                                    <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
                                        <Text numberOfLines={2} style={{ fontSize: 17, height: 30, lineHeight: 30 }}>
                                            {item.investmentName + ' ' + item.spec + '' + item.specUnit + ' /' + item.packUnit}
                                        </Text>
                                        <Text style={{ color: '#49a9ee', lineHeight: 30, }}>{item.investmentTypeStr}</Text>
                                    </View>
                                    <WhiteSpace />
                                    <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
                                        <Text style={{ textAlign: 'right', lineHeight: 30, marginRight: 10 }}>数量：{item.quantity}</Text>
                                        <Text style={{ color: '#49a9ee', lineHeight: 30, }}>{item.buyDateStr}</Text>
                                    </View>
                                    <WhiteSpace />
                                </View>
                            </TouchableWithoutFeedback>
...
module.exports = connect(mapStateToProps)(IntBuyRecord);
