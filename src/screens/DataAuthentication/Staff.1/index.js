                                ...
                                onPress: () => {
                                    Alert.alert(
                                        ...
                                        },
                                        {
                                            text: '确定',
                                            onPress: () => {
                                                MineAction.StaffDelete( {idList: [item.id]} ).then((data) => {
                                                    if (data.suc) {
                                                        this.refresh()
                                                    } else {
                            ...
                            <TouchableWithoutFeedback activeOpacity={0.3} underlayColor={'#eee'}
                                ...
                                <View style={{ borderBottomColor: '#e3e3e3', borderBottomWidth: 1, paddingLeft: 12, paddingRight: 12, paddingTop: 10 }}>
                                    ...
                                    <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
                                        <Text style={{ textAlign: 'right', lineHeight: 30, marginRight: 10 }}>{item.phone}</Text>
                                        <Text style={{ color: '#49a9ee', lineHeight: 30, }}>{item.typeStr }</Text>
                                    </View>
                                    <WhiteSpace />
                                </View>
                            </TouchableWithoutFeedback>
...
module.exports = connect(mapStateToProps)(Staff);
