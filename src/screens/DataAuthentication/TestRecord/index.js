...
@navigatorDecorator
class TestRecord extends Component {
    constructor(props) {
        super(props);
        this.state = {
            publishState: [''],
                                ...
                                onPress: () => {
                                    Alert.alert(
                                        ...
                                        },
                                        {
                                            text: '确定',
                                            onPress: () => {
                                                MineAction.DeleteUse([item.id]).then((data) => {
                                                    if (data.suc) {
                                                        this.refresh()
                                                    } else {
                                ...
                                onPress={() => {
                                    this.pushPage({
                                        component: {
                                            ...
                                            ...Global.Screens.TestRecordDetail,
                                        }
                                    });
                                }}>
                                <View style={{ borderBottomColor: '#e3e3e3', borderBottomWidth: 1, paddingLeft: 12, paddingRight: 12, paddingTop: 10 }}>
                                    <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
                                        <Text numberOfLines={2} style={{ fontSize: 17, height: 30, lineHeight: 30 }}>{item.cropName + '/' + item.breedName}</Text>
                                        <Text numberOfLines={2} style={{ fontSize: 17, height: 30, lineHeight: 30 }}>{item.examinDay}</Text>
                                    </View>
                                    <WhiteSpace />
                                    <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
                                        <Text style={{ textAlign: 'right', lineHeight: 30, marginRight: 10 }}>检测结果：{item.examining == 1 ? '合格' : '不合格'}</Text>
                                        <Text style={{ color: '#49a9ee', lineHeight: 30, }}>{item.examinTypeDisplay}</Text>
                                    </View>
                                    <WhiteSpace />
                                </View>
                            </TouchableWithoutFeedback>
...
module.exports = connect(mapStateToProps)(TestRecord);
