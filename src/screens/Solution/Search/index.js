                    ...
                    <View style={styles.searchBtn}>
                        {
                            this.state.typeList && this.state.typeList.length > 0 ? this.state.typeList.map((item, index) => {
                                return <Button  activeStyle={{opacity:0.5,backgroundColor:'none'}}key={index} style={[styles.searchBtns, { backgroundColor: this.state.classificCode == item.value ? 'green' : '#fff', }]} value={item.value} onClick={() => this.selectType(item.value)}><Text style={{ color: this.state.classificCode == item.value ? '#fff' : '#222' }}>{item.label}</Text></Button>
                            }) : <Text>暂无数据</Text>
                        }
                    </View>
                    <View style={styles.searchBtn}>
                        {
                            this.state.focusList && this.state.focusList.length > 0 ? this.state.focusList.map((item, index) => {
                                return <Button  activeStyle={{opacity:0.5,backgroundColor:'none'}}key={index} style={[styles.searchBtns, { backgroundColor: this.state.focus == item.value ? 'blue' : '#fff', }]} value={item.value} onClick={() => this.selectFocus(item.value)}><Text style={{ color: this.state.focus == item.value ? '#fff' : '#222' }}>{item.label}</Text></Button>
                            }) : <Text>暂无数据</Text>
                        }
                    </View>
                </View>
