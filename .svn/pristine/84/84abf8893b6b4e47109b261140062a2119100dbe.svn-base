...
class ControlAdd extends Component {
    ...
    navigationButtonPressed() {
            ...
            i.children.forEach((o) => {
                ...
                if (!o.type) {
                    ...
                    if (o.type == 1) {
                        if (!o.str) {
                            nowName = true
                            MyToast.info('请选择负面清单');
                        } else if (!o.area) {
                            nowName = true
                            MyToast.info('请填写回收率');
                        } else if (!ret.test(o.area)) {
                            nowName = true
                            MyToast.info('回收率应大于0，且最多两位小数的数字');
                        }
                    } else {
                        if (!o.str) {
                            nowName = true
        ...
        if (nowName) {
            ...
            if (this.props.cropName) {
                postData.cropName = this.props.cropName
            ...
            MineAction.ControlDetail({
                ...
                if (data.suc) {
                    ...
                    model.items.forEach((i, ind) => {
                        ...
                        i.str = [i.controlCategoryCode + '*' + i.controlCategoryName + '*' + i.parentCategoryCode + '*' + i.type]
                        i.children.forEach((o, index) => {
                            o.str = [o.controlCategoryCode + '*' + o.controlCategoryName + '*' + o.parentCategoryCode + '*' + o.type]
                        })
                    })
                    this.setState({
                        cropState: [model.cropName],
                        startTime: new Date(model.year + '-' + model.month + '-01'),
                        items: model.items
                    })
                } else {
        ...
        MineAction.GetAgriPreventionControlClass({ input: postData }).then((data) => {
            if (data.suc) {
                ...
                model.forEach((i, index) => {
                    ...
                    arrInt[index].value = i.code + "*" + i.name + '*' + (i.parentCode ? i.parentCode : 'null') + '*' + (i.type ? (+ i.type) : 'null')
                });
                if (code == 'all') {
                    this.setState({
                        controlList: arrInt,
    ...
    changeData(name, index, data, childrenIndex) {
        ...
        if (name == 'controlState') {
            if (data.length < 1 || data[0] == dataList[index].str) {
                return false
            }
            let str = data[0]
            ...
            dataList[index]['type'] = str.split('*')[3]
        ...
        if (name == 'typeState') {
            ...
            dataList[index].children[childrenIndex]['type'] = str.split('*')[3]
        }
        if (name == 'area') {
            ...
            <ScrollView style={{ backgroundColor: '#f5f5f5' }}>
                ...
                <View>
                                        ...
                                        i.children.map((o, index) => {
                                            return (
                                                <View key={index}>
                                                    <List>
                                                        <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
                                                            <Text style={{ lineHeight: 30, fontSize: 16, marginLeft: 15, color: 'green', fontWeight: 'bold' }}>
                                                                {
                                                                    (!i.type || i.type == 'null' ? ('技术措施（') : ('负面清单（')) + (index + 1) + '）'
                                                                }
                                                            </Text>
                                                            {this.state.items[ind].children.length == 1 ? (null) : (
                                                                <TouchableWithoutFeedback onPress={() => {
                                                                    let nowList = lodash.cloneDeep(this.state.items)
                                                    ...
                                                    <List>
                                                        <Picker
                                                            ...
                                                            style={{ color: '#000' }}
                                                        >
                                                            <Item><Text style={{ fontSize: 17, color: '#000' }}>技术</Text></Item>
                                                        </Picker>
                                                    </List>
                                                    {/* 判断是否是科学技术  1使用率 2是否*/}
                                                    {
                                                        this.state.items[ind].children[index].type || this.props.cropName ? (
                                                            this.state.items[ind].children[index].type == 1 ? (
                                                                <List>
                                                                    <InputItem
                                                                        labelNumber={12}
                                                                        textAlign="right"
                                                                    ...
                                                                    this.state.items[ind].children[index].type == 2 ? (
                                                                        <List>
                                                                            <Picker
                                                                                registerNumber="是否"
                                                                                data={this.state.isList}
                                                                                cols={1}
                                                                                value={this.state.items[ind].children[index].area ? [this.state.items[ind].children[index].area] : ''}
                                                                                onOk={(val) => {
                                                                                    this.changeData('area', ind, val[0], index)
                                                                                }}
                                                                                style={{ color: '#000' }}
                                                                            >
                                                                                <Item><Text style={{ fontSize: 17, color: '#000' }}>结果</Text></Item>
                                                                            </Picker>
                                                                        </List>
                                                                    ) : (
                                                                            <List>
                    ...
                    <TouchableWithoutFeedback onPress={() => {
                        ...
                        <View>
                            <Text style={{ textAlign: 'center', fontSize: 18, color: '#00BFFF', lineHeight: 50 }}><Icon name='plus-circle' size={18} />添加防控类型</Text>
                        </View>
                    </TouchableWithoutFeedback>
                </View>
            </ScrollView >
