...
import { Button, SwipeAction, Checkbox } from 'antd-mobile-rn';
...
export default class ERPDiaPest extends Component {
    constructor(props) {
        ...
        this.nowSelect = []
        this.state = {
            pestList: this.props.pestList != "" && this.props.pestList != null ? this.props.pestList : [],
            index: 0,
            routes: [
    ...
    queryDisease = () => {
        DiagnosisAction.GetPestByCrops({ input: { type: '病害', crops: (this.props.crop ? [this.props.crop] : null) } }).then(({ suc, data }) => {
            this.setState({
                loading: false
            })
    ...
    queryInsect = () => {
        DiagnosisAction.GetPestByCrops({ input: { type: '虫害', crops: (this.props.crop ? [this.props.crop] : null) } }).then(({ suc, data }) => {
            if (suc && data && data.length > 0) {
                this.setState({
                    insectList: data
                })
    ...
    queryGrass = () => {
        DiagnosisAction.GetPestByCrops({ input: { type: '草害', crops: (this.props.crop ? [this.props.crop] : null) } }).then(({ suc, data }) => {
            if (suc && data && data.length > 0) {
                this.setState({
                    grassList: data
                })
