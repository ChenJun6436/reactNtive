...
@loadingDecorator
export default class GardenCropDetail extends Component {
    constructor(props) {
        super(props);
        this.state = {
            model: {
        ...
        return (
            <ScrollView style={{ backgroundColor: '#f2f2f2', position: 'relative', height: SCREEN_HEIGHT }}>
                <View style={styles.container}>
                    ...
                    <DetailItem title="开启状态" content={model.stateStr} />
                    <DetailItem title="作物类型" content={model.breedName} />
                    <DetailItem title="苗木来源" content={model.breedSource} />
                    <DetailItem title="基地名称" content={model.plantBaseName} />
                    <DetailItem title="园区名称" content={model.plantGardenName} />
                    <DetailItem title="地块名称" content={model.plantBaseLandName} />
