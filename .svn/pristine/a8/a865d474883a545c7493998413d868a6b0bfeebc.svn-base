...
@navigatorDecorator
export default class LandManageGardenAdd extends Component {
    constructor(props) {
        super(props);
        this.loading = false
        this.state = {
    ...
    navigationButtonPressed({ buttonId }) {
        ...
        let postData = {
            plantBaseId: this.props.baseId,
            lands: landList,
        }
        MineAction.AddLand({ input: postData }).then((data) => {
            if (data.suc == 1) {
                MyToast.success('新增成功');
                this.props.callback(this.props.baseId)
                setTimeout(() => {
                    this.pop();
                }, 2000)
            } else {
