                                ...
                                onPress={() => {
                                    this.pushPage({
                                        component: {
                                            ...
                                            ...Global.Screens.WorkRecordDetail,
                                        }
                                    });
                                }}>
                                <View style={{ borderBottomColor: '#e3e3e3', borderBottomWidth: 1, paddingLeft: 12, paddingRight: 12, paddingTop: 10 }}>
                                    <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
                                        <Text numberOfLines={2} style={{ fontSize: 17, height: 30, lineHeight: 30 }}>基地/区块名称:{item.plantBaseName + '/' + item.plantBaseLandName}</Text>
                                        <Text numberOfLines={2} style={{ fontSize: 17, height: 30, lineHeight: 30 }}>产品名称:{item.cropName}</Text>
                                    </View>
                                    <WhiteSpace />
                                    <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
                                        <Text style={{ textAlign: 'right', lineHeight: 30, marginRight: 10 }}>用工数量(人/天):{item.workDays}</Text>
                                        <Text style={{ color: '#49a9ee', lineHeight: 30, }}>成本(元):{item.workCost}</Text>
                                    </View>
                                    <WhiteSpace />
                                </View>
                            </TouchableWithoutFeedback>
...
module.exports = connect(mapStateToProps)(WorkRecord);
