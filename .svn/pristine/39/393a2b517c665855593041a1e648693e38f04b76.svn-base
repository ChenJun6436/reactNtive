...
@navigatorDecorator
class CureList extends Component {
    constructor(props) {
        super(props);
        this.state = {
            publishState: [''],
        ...
        return (
            <View style={styles.container}>
                    ...
                    <View>
                        <DatePicker
                            ...
                            onChange={text => this.setState({ year: text }, () => {
                                this.refresh({ year: moment(this.state.year).format("YYYY") , })
                                ; })}
                            format={val => moment(val).format("YYYY") }
                        >
                            <Item arrow="horizontal" ><Text style={{ color: 'gray' }}>时间选择</Text></Item>
                        </DatePicker>
                    </View>
                    {/* <View>
                        <DatePicker
                ...
                <LoadMoreList
                    ...
                    getData={MineAction.cureList}
                    searchParams={{ keyword: '', year: this.state.year }}
                    rowItem={(item) => {
                            ...
                            <TouchableWithoutFeedback activeOpacity={0.3} underlayColor={'#eee'}
                                ...
                                <View style={{ borderBottomColor: '#e3e3e3', borderBottomWidth: 1, paddingLeft: 12, paddingRight: 12, paddingTop: 10 }}>
                                    ...
                                    <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
                                        ...
                                        <Text  style={{ fontSize: 17, height: 30, lineHeight: 30 }}>
                                            {item.year}
                                        </Text>
                                    </View>
                                    <WhiteSpace />
                                    <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
                                        <Text numberOfLines={2} style={{ fontSize: 17, height: 30, lineHeight: 30 }}>
                                            防治面积： {item.totalArea} 亩
                                        </Text>
                                        <Text style={{ color: '#49a9ee', lineHeight: 30, }}>防治费用： {item.totalCost} 元</Text>
                                    </View>
                                    <WhiteSpace />
                                </View>
                            </TouchableWithoutFeedback>
...
module.exports = connect(mapStateToProps)(CureList);
